namespace SMPL
{
	public class Scene
	{
		public struct TexturedModel3D
		{
			...
			public TexturedModel3D(string objPath, string texturePath, string uniqueName = null, uint textureCount = 20,
				float textureDetail = 20, float scaleX = 1, float scaleY = 1, float scaleZ = 1)
			{
		...
		public Dictionary<string, SFML.Graphics.Font> Fonts { get; } = new();
		...
		internal void LoadAssets()
		{
			...
			//	catch (Exception)
			//	{ Sprites3D[assets.TexturedModels3D[i].UniqueName] = null; Debug.LogError(-1, $"Could not load textured 3D model '{assets.TexturedModels3D[i]}'."); }
			//	UpdateLoadingPercent();
			//}
